...
const EnvironmentIndicator: React.FC<EnvironmentIndicatorProps> = ({ environment, onClick }) => {
  ...
  if (isMobile) {
    return (
      <div className="fixed top-6 left-6 z-[90] flex flex-col items-start gap-3">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          onClick={onClick}
          className="flex items-center gap-3 bg-white/60 backdrop-blur-md border border-[#D4AF37]/20 shadow-lg rounded-full px-4 py-2 cursor-pointer"
        >
          <span className="calligraphy text-lg text-[#B22222]">
          ...
          <span className="text-[10px] font-black text-[#B22222] tracking-widest uppercase">
        ...
        {/* Mobile Daily Zen Icon - Positioned below weather */}
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.3 }}
        >
          <DailyZenFloating isMobileMode={true} />
        </motion.div>
      </div>
    );
  }
